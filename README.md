...
The goal of the paper was to train a neural network to predict the next word given the previous *n* words. Simple enough. There is an interesting byproduct that occurs too. The model learns **word embeddings** — dense vector representations of words where semantically similar words (like *pencil* and *pen*) end up close together in vector space.

Before embeddings, the standard approach was one-hot encoding: a massive vector with a 1 in one position and 0s everywhere else. For a vocab of 10,000 words that's a 10,000-dimensional input, and *shirt* and *pants* are just as "similar" as *pants* and *lamp*. Embeddings solve both problems using smaller dimenional representation and semantic meaning baked in the vectors.
...
├── notebooks/
│   └── notebook.ipynb       # step-by-step walkthrough with math + manual backprop
...
The `notebooks/` folder is worth a look if you want to see the full derivation of each gradient — it walks through every step of the backward pass with shape annotations.
...
- [PTB via Tomáš Mikolov's page](http://www.fit.vutbr.cz/~imikolov/rnnlm/simple-examples.tgz) — extract and use `ptb.train.txt`

Once downloaded, place it at `data/text_chunk.txt`.
